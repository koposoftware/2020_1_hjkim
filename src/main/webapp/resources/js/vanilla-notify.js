var vNotify = (function() {
	...
	var addNotify = function(params) {
		...
		/*if (params.click) {
			item.appendChild(addClick(params.click));
		}*/
	...
	/*var addClick = function(click) {
		var item = document.createElement('div');
		item.classList.add('vnotify-text');
		item.innerHTML = "상담페이지로 바로가기"
		item.addEventListener('click', function() {
			location.href = click;
		});
		return item;
	...
	var fade = function(type, ms, el) {
		var isIn = type === 'in', opacity = isIn ? 0 : el.style.opacity || 1, goal = isIn ? 0.8 : 0, gap = options.fadeInterval / ms;
