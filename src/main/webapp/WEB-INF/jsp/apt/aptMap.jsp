	...
	<section>
		<div class="page-wrapper chiller-theme toggled">
			...
			<main class="page-content margin-top-80">
				<div class="container-fluid apt-detail" style="display: none;"></div>
	...
	<script>
		...
		function basketCheck(kaptCode) {
			...
			if (loginVO == "") {
				...
				$.ajax({
					url : '${ pageContext.request.contextPath }/apt/basketCheck',
					type : 'post',
					data : {
						kaptCode : kaptCode,
		...
		function displayInfo(infomarker, infodata) {
			var basketDetailCheck = basketCheck(infodata.kaptCode)
			$('#basketIcon' + infodata.kaptCode).attr('class', basketDetailCheck)
			$.ajax({
				...
				success : function(data) {
					...
					/* content += '&nbsp;&nbsp;<i class="' + basketCheck(infodata.kaptCode) + '" onclick="basket(\'' + infodata.kaptCode + '\',this)"></i>' */
					content += '&nbsp;&nbsp;<i id="basketIcon'+ infodata.kaptCode +'" onclick="basket(\'' + infodata.kaptCode + '\',this)"></i>'
					content += '            <div class="close" onclick="closeCustomOverlay()" title="닫기"></div>'
					content += '        </div>'
					content += '        <div class="body">'
					content += '            <div class="desc">'
		...
		function basket(kaptCode, toggleCheck) {
			var checkIcon = toggleCheck.classList
			<c:choose>
			<c:when test="${ empty loginVO }">
			loginCheck()
			</c:when>
			<c:otherwise>
			if (checkIcon.value == "far fa-heart") {
				$.ajax({
					url : '${ pageContext.request.contextPath }/apt/addBasket',
					type : 'post',
					data : {
						kaptCode : kaptCode,
						userNo : '${loginVO.userNo}'
					...
					success : function() {
						myFunction(toggleCheck)
						addMarker(bounds)
					}
				})
			} else {
				$.ajax({
					url : '${ pageContext.request.contextPath }/apt/removeBasket',
					type : 'post',
					data : {
						kaptCode : kaptCode,
						userNo : '${loginVO.userNo}'
